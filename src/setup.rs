...
use stm32f767_hal::gpio::{
    gpioa::{Parts as GpioAParts, PA1, PA2, PA7},
    gpiob::{Parts as GpioBParts, PB13},
    gpioc::{Parts as GpioCParts, PC1, PC4, PC5},
    gpiog::{Parts as GpioGParts, PG11, PG13},
    Speed::VeryHigh,
};
use stm32f767_hal::stm32f7x7::{RCC, SYSCFG};
...
#[cfg(feature = "nucleo-f767zi")]
pub fn setup_pins<M1, M2, M3, M4, M5, M6, M7, M8, M9>(
    gpioa: &mut GpioAParts,
    gpiob: &mut GpioBParts,
    gpioc: &mut GpioCParts,
    gpiog: &mut GpioGParts,
    pa1: PA1<M1>,
    pa2: PA2<M2>,
    pa7: PA7<M3>,
    pb13: PB13<M4>,
    ...
    // PA1 RMII Reference Clock - SB13 ON
    pa1.into_af11(&mut gpioa.moder, &mut gpioa.afrl)
        .set_speed(VeryHigh);
    // PA2 RMII MDIO - SB160 ON
    pa2.into_af11(&mut gpioa.moder, &mut gpioa.afrl)
        .set_speed(VeryHigh);
    // PC1 RMII MDC - SB164 ON
    pc1.into_af11(&mut gpioc.moder, &mut gpioc.afrl)
        .set_speed(VeryHigh);
    // PA7 RMII RX Data Valid D11 JP6 ON
    pa7.into_af11(&mut gpioa.moder, &mut gpioa.afrl)
        .set_speed(VeryHigh);
    // PC4 RMII RXD0 - SB178 ON
    pc4.into_af11(&mut gpioc.moder, &mut gpioc.afrl)
        .set_speed(VeryHigh);
    // PC5 RMII RXD1 - SB181 ON
    pc5.into_af11(&mut gpioc.moder, &mut gpioc.afrl)
        .set_speed(VeryHigh);
    // PG11 RMII TX Enable - SB183 ON
    pg11.into_af11(&mut gpiog.moder, &mut gpiog.afrh)
        .set_speed(VeryHigh);
    // PG13 RXII TXD0 - SB182 ON
    pg13.into_af11(&mut gpiog.moder, &mut gpiog.afrh)
        .set_speed(VeryHigh);
    // PB13 RMII TXD1 I2S_A_CK JP7 ON
    pb13.into_af11(&mut gpiob.moder, &mut gpiob.afrh)
        .set_speed(VeryHigh);
}
